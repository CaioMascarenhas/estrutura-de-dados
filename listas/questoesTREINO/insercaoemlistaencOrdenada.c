...
if (novo_no->item.chave == l->cabeca->item.chave) return false;
if (novo_no->item.chave < l->cabeca->item.chave)
{
...
while(aux->prox != NULL)
{
  if (novo_no->item.chave == aux->prox->item.chave) return false;
  if (novo_no->item.chave < aux->prox->item.chave)
  {
