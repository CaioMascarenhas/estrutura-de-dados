...
// Verifica se h� espa�o suficiente em l1 para os elementos de l2
    if (tamanho(*l1) + tamanho(*l2) > MAX)
        return false; // N�o h� capacidade suficiente em l1

    // Insere cada elemento de l2 em l1
    for (int i = 0; i < tamanho(*l2); i++)
    {
        ITEM item = enesimo(i, *l2);
        inserir(item, l1);
    }

    return true; // Concatena��o bem-sucedida
